...
class RoundRobin:
    def __init__(self, servers):
        self.servers = servers
        self.idx_nextServer=0

    def select_server(self):
        server = self.servers[self.idx_nextServer]
        self.idx_nextServer+=1
        if self.idx_nextServer >= len(self.servers):
            self.idx_nextServer=0
        return server
...
class LeastResponseTime:
    ...
    def select_server(self):
        server = min(self.avgTime, key=self.avgTime.get)
        self.time[server] = time.time()
        return server


    def update(self, *arg):
        self.time[arg[0]] = time.time() - self.time[arg[0]]
        server_times = {}
        for server in self.time:  
            if server not in server_times:
                server_times[server] = [self.time[server]]
            else:
                server_times[server].append(self.time[server])
        for server in server_times:
            sum_times = sum(server_times[server])
            self.avgTime[server] = sum_times / len(server_times[server])
...
class SocketMapper:
    ...
    def delete(self, sock):
        if not mapper.get_upstream_sock(sock):
            policy.update(sock.getpeername())
