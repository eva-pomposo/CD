...
class FingerTable:
    ...
    def update(self, index, node_id, node_addr):
        ...
        if (len(self.finger) > index-1):
            self.finger.pop(index-1)
        self.finger.insert(index-1, (node_id, node_addr))

    def find(self, identification):
        """ Get node address of closest preceding node (in finger table) of identification. """
        for i in range (len(self.finger)):
            if (contains(self.node_id,self.finger[i][0],identification)):
    ...
    def refresh(self):
        lista = []
        for i in range (len(self.finger)):
            lista.append((i+1, (self.node_id + 2**i) % 2**self.m_bits, self.finger[i][1]))
        return lista
...
class DHTNode(threading.Thread):
    ...
    def get_successor(self, args):
        ...
        if (contains(self.identification, self.successor_id, id)):
            msg = {"method": "SUCCESSOR_REP", "args": {"req_id": id, "successor_id" : self.successor_id, "successor_addr": self.successor_addr}}
            self.send(addr,msg)
        elif (self.predecessor_id == None or contains(self.predecessor_id,self.identification,id)):
            msg = {"method": "SUCCESSOR_REP", "args": {"req_id": id, "successor_id" : self.identification, "successor_addr": self.addr}}
            self.send(addr,msg)
        else:
            msg = {"method": "SUCCESSOR", "args": {"id": id, "from" : addr}}
    ...
    def get(self, key, address):
        ...
        if (contains(self.predecessor_id, self.identification,key_hash)):
            ...
            msg = {"method": "GET", "args": {"key": key, "from": address}}
            self.send(self.finger_table.find(self.successor_id), msg)              
        ...
        while not self.done:
            ...
            if payload is not None:
                ...
                if output["method"] == "JOIN_REQ":
                    ...
                    self.finger_table.update(self.finger_table.getIdxFromId(output["args"]["req_id"]),successor_id,successor_addr)
